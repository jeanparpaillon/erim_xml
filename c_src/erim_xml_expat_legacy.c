...
#define	DRIVER_NAME	erim_xml_expat_legacy
...
static make_attributes_legacy_cb erim_xml_cb_make_attributes;

static int		create_parser(struct erim_xml_data *edd);
static void		init_parser(struct erim_xml_data *edd);
...
erim_xml_start(ErlDrvPort port, char *command)
{
	...
	edd->ctx.make_attributes_legacy = erim_xml_cb_make_attributes;
...
int
erim_xml_cb_make_attributes(struct erim_xml_ctx *ctx, void *attributes)
{
	int i;
	const char **attrs;
...
static void
init_parser(struct erim_xml_data *edd)
{
