...
#define	DRIVER_NAME	erim_xml_libxml2

/* Driver data (also, user data for LibXML2). */
struct erim_xml_data {
	/* Generic context. */
	struct erim_xml_ctx	  ctx;
...
static make_declared_nss_cb	erim_xml_cb_make_declared_nss;
static make_attributes_cb	erim_xml_cb_make_attributes;

static int		create_parser(struct erim_xml_data *edd);
static void		destroy_parser(struct erim_xml_data *edd);
...
static ErlDrvData
erim_xml_start(ErlDrvPort port, char *command)
{
	struct erim_xml_data *edd;
	...
	edd->ctx.make_declared_nss = erim_xml_cb_make_declared_nss;
	edd->ctx.make_attributes = erim_xml_cb_make_attributes;
...
static void
erim_xml_stop(ErlDrvData drv_data)
{
	struct erim_xml_data *edd;

	edd = (struct erim_xml_data *)drv_data;
...
static ErlDrvSSizeT
erim_xml_control(ErlDrvData drv_data, unsigned int command,
    char *buf, ErlDrvSizeT len, char **rbuf, ErlDrvSizeT rlen)
{
	...
	default:
		/* Other commands are handled in 'erim_xml.c' */
		to_return = erim_new_xbuf();
		if (to_return == NULL)
			return (-1);
    ...
    int attrs_count, int attrs_defaulted, const xmlChar **attrs)
{
	...
	edd = (struct erim_xml_data *)user_data;
...
static int
erim_xml_cb_make_declared_nss(struct erim_xml_ctx *ctx, void *user_data)
{
	int n, i;
...
static int
erim_xml_cb_make_attributes(struct erim_xml_ctx *ctx, void *user_data)
{
	int n, i, value_len;
...
static void
destroy_parser(struct erim_xml_data *edd)
{
...
DRIVER_INIT(DRIVER_NAME)
{
	...
	driver_entry.init = erim_xml_init;
	driver_entry.finish = erim_xml_finish;
	driver_entry.start = erim_xml_start;
	driver_entry.stop = erim_xml_stop;
	driver_entry.control = erim_xml_control;
