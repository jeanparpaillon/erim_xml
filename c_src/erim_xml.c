...
/* Global known lists. */
static struct erim_hashtable	*known_nss_index = NULL;
static struct erim_hashtable	*known_elems_index = NULL;
...
static int		select_known_nss(struct erim_xml_ctx *ctx,
			    const char *buf, int index);
static int		select_known_elems(struct erim_xml_ctx *ctx,
			    const char *buf, int index);
static int		is_known_ns(struct erim_xml_ctx *ctx,
			    const char *ns, int ns_len);
static int		is_known_elem(struct erim_xml_ctx *ctx,
			    const char *elem, int elem_len);

static int		encode_ns(struct erim_xml_ctx *ctx,
			    ei_x_buff *tree, const char *ns, int ns_len);
static int		encode_elem(struct erim_xml_ctx *ctx,
			    ei_x_buff *tree, const char *elem, int elem_len);
static int		encode_attr(struct erim_xml_ctx *ctx,
			    ei_x_buff *tree, const char *attr, int attr_len);

static int		current_tree_finished(struct erim_xml_ctx *ctx);
static void		reset_current_tree(struct erim_xml_ctx *ctx);
...
init_known_lists()
{
	 ...
	 * associated locks. */
	known_nss_index = erim_ht_create(4,
	    (void (*)(void *))erim_ht_destroy);
	if (known_nss_index == NULL)
		goto err;

	known_elems_index = erim_ht_create(4,
	    (void (*)(void *))erim_ht_destroy);
...
err:
	if (known_nss_index != NULL) {
		erim_ht_destroy(known_nss_index);
		known_nss_index = NULL;
	}

	if (known_elems_index != NULL) {
		erim_ht_destroy(known_elems_index);
...
int
control(struct erim_xml_ctx *ctx, unsigned int command,
...
int
init_context(struct erim_xml_ctx *ctx)
{
...
void
reset_context(struct erim_xml_ctx *ctx)
{
...
void
free_context(struct erim_xml_ctx *ctx)
{
...
void
add_data_size(struct erim_xml_ctx *ctx, long size)
{
...
int
is_data_size_under_limit(struct erim_xml_ctx *ctx, long size)
{
...
int
enter_element(struct erim_xml_ctx *ctx,
    const char *ns, int ns_len,
    const char *elem, int elem_len,
...
int
make_declared_ns_in_buf(struct erim_xml_ctx *ctx, ei_x_buff *buf,
    const char *ns, int ns_len,
...
int
make_declared_ns(struct erim_xml_ctx *ctx,
    const char *ns, int ns_len,
    const char *prefix, int prefix_len)
{
	int ret;
...
int
make_attribute(struct erim_xml_ctx *ctx,
    const char *ns, int ns_len,
...
int
add_character_data(struct erim_xml_ctx *ctx,
...
int
exit_element(struct erim_xml_ctx *ctx,
    const char *ns, int ns_len,
    const char *elem, int elem_len)
{
	ei_x_buff *tree;
...
void
reset_complete_trees(struct erim_xml_ctx *ctx)
{
...
int
enter_element_legacy(struct erim_xml_ctx *ctx,
    const char *elem, int elem_len,
    void *attributes)
{
	int ret;
...
int
make_attribute_legacy(struct erim_xml_ctx *ctx,
    const char *attr, int attr_len,
    const char *value, int value_len)
{
	ei_x_buff *tree;
...
int
exit_element_legacy(struct erim_xml_ctx *ctx)
{
	ei_x_buff *tree;
...
static struct erim_hashtable *
lookup_known_nss(const char *list_name, int list_name_len)
{
	struct erim_hashtable *kl;

	if (known_nss_index == NULL)
		return (NULL);

	/* Lookup the known list. */
	kl = erim_ht_fetch(known_nss_index, list_name, list_name_len);
	if (kl == NULL) {
		/* This list doesn't exist yet, create it. */
		kl = erim_ht_create(KNOWN_NSS_AVG_MIN_LENGTH, NULL);
		if (kl == NULL)
			return (NULL);
...
lookup_known_elems(const char *list_name, int list_name_len)
{
	...
	/* Lookup the known list. */
	kl = erim_ht_fetch(known_elems_index, list_name, list_name_len);
	if (kl == NULL) {
		/* This list doesn't exist yet, create it. */
		kl = erim_ht_create(KNOWN_ELEMS_AVG_MIN_LENGTH, NULL);
		if (kl == NULL)
			return (NULL);

		/* Add it to the lists index. */
		erim_ht_store(known_elems_index,
		    list_name, list_name_len, kl);
	}
...
static int
update_list(struct erim_hashtable *kl, const char *buf, int *index)
{
	...
	for (i = 0; i < nb_items; ++i) {
		...
		/* Add it to the list. */
		if (!erim_ht_exists(kl, item, item_len))
			erim_ht_store(kl, item, item_len, &DUMMY);
	}
...
static int
select_known_nss(struct erim_xml_ctx *ctx, const char *buf, int index)
{
...
static int
select_known_elems(struct erim_xml_ctx *ctx, const char *buf, int index)
{
	int type, list_name_len;
	char list_name[MAXATOMLEN];
	struct erim_hashtable *kl;
...
static int
is_known_ns(struct erim_xml_ctx *ctx, const char *ns, int ns_len)
{
	int is_known;
	...
	is_known = erim_ht_exists(ctx->known_nss, ns, ns_len);

	return (is_known);
}

static int
is_known_elem(struct erim_xml_ctx *ctx, const char *elem, int elem_len)
{
	int is_known;
	...
	is_known = erim_ht_exists(ctx->known_elems, elem, elem_len);
...
static int
encode_ns(struct erim_xml_ctx *ctx, ei_x_buff *tree,
    const char *ns, int ns_len)
{
	int ret;
...
static int
encode_elem(struct erim_xml_ctx *ctx, ei_x_buff *tree,
    const char *elem, int elem_len)
{
	int ret;
...
static int
encode_attr(struct erim_xml_ctx *ctx, ei_x_buff *tree,
    const char *attr, int attr_len)
{
	int ret;
...
static int
current_tree_finished(struct erim_xml_ctx *ctx)
{
	ei_x_buff *trees;
...
static void
reset_current_tree(struct erim_xml_ctx *ctx)
{
