...
-define(ENGINES_REGISTRY, erim_xml_engines_registry).
...
-define(REGISTER_LIBXML2,
	register_builtin_engine(libxml2, erim_xml_libxml2)).
-else.
...
load_builtin_known_lists() ->
    [_ | Known_NSs] = lists:reverse(?XML_KNOWN_NSS),
    [_ | Known_Elems] = lists:reverse(?XML_KNOWN_ELEMS),
...
get_cdata(undefined) ->
    ...
    %% erim_xml:get_cdata(erim_xml:get_element(XML_El, body))
    <<>>.
...
unresolve_xmlel_nss(#xmlel{ns = NS, name = Name, attrs = Attrs,
    ...
    Name_S = erim_known_elems:elem_as_list(Name),
...
unresolve_endtag_nss(#xmlendtag{ns = NS, name = Name},
		     Default_NS, Prefixed_NS) ->
    Name_S = erim_known_nss:ns_as_list(Name),
...
xmlnsattributes_to_xmlattributes2([#xmlattr{ns = NS, name = Name,
	...
	case NS of
		...
		case search_in_prefixed_ns(NS, Prefixed_NS) of
			...
			NS_S = erim_known_nss:ns_as_list(NS),
...
forward_declare_ns(Curr_NS, [{NS, none} | Rest],
    ...
    %% XXX This would benefit the addition of erim_known_*:*_as_binary/1.
    NS_S = erim_known_nss:ns_as_list(NS),
...
forward_declare_ns(Curr_NS, [], Attrs, Default_NS, Prefixed_NS) ->
    ...
    case Use_Default_NS of
	    ...
	    %% We look for a prefixed namespace.
		    ...
		    %% XXX This would benefit the addition of
		    %% erim_known_*:*_as_binary/1.
                    Curr_NS_S = erim_known_nss:ns_as_list(Curr_NS),
...
element_to_iolist(Name, Attrs, Children, Default_NS, Prefixed_NS)
  when is_atom(Name) ->
    element_to_iolist(erim_known_elems:elem_as_list(Name), Attrs, Children,
		      Default_NS, Prefixed_NS);
...
endtag_to_iolist(Name) when is_atom(Name) ->
    endtag_to_iolist(erim_known_elems:elem_as_list(Name));
endtag_to_iolist(Name) ->
    [$<, $/, Name, $>].
